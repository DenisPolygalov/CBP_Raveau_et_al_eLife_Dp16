...
% Spike passes condition if it is included in a set 
% of N spikes with ISI_N <= threshold.
...
for i = N:length(Spike.T)
   if INBURST == 0           % Was not in burst.
      ...
      elseif diff(Spike.T([i-(N-1) i])) > ISI_N && BL >= N
         ...
         %
         % Skip this if at the start of a new burst (i.e. 'BL >= N' requirement).
         %
   ...
   SpikeBurstNumber(i) = NUMBER; % Assign a burst number to each spike.
end
